...
class TechFetch(Scraper):
    def __init__(self, proxies: list[str] | str | None = None, ca_cert: str | None = None, user_agent: str | None = None):
        super().__init__(Site.TECHFETCH, proxies=proxies, ca_cert=ca_cert, user_agent=user_agent)
        self.base_url = "https://www.techfetch.com"
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        url = f"{self.base_url}/job-openings/{q}/{l}"
        jobs: list[JobPost] = []
        ...
        try:
            ...
            if response.ok:
                ...
                if "captcha" not in html.lower() and "access denied" not in html.lower():
                    ...
                    log.warning("TechFetch returned anti-bot/challenge page; skipping in legal no-key mode.")
            else:
                log.warning(f"TechFetch response status code {response.status_code}")
        except Exception as exc:
            log.warning(f"TechFetch request failed: {exc}")
    ...
    def _parse_html(self, html: str) -> list[JobPost]:
        ...
        for script in soup.find_all("script", attrs={"type": "application/ld+json"}):
            ...
            for item in items:
                ...
                jobs.append(
                    JobPost(
                        id=f"techfetch-{abs(hash(job_url))}",
                        title=title,
                        company_name=None,
                        job_url=job_url,

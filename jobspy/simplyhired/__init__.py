...
class SimplyHired(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        for page in range(1, max_pages + 1):
            ...
            url = f"{self.base_url}/search?q={query}&l={where}&pn={page}"
            ...
            try:
                ...
                log.warning(f"SimplyHired request failed on page {page}: {exc}")
                break

            if not response.ok:
                log.warning(f"SimplyHired response status code {response.status_code}")
                break

            html = response.text
            if "enable javascript" in html.lower() and "captcha" in html.lower():
                log.warning("SimplyHired returned anti-bot challenge page; skipping in legal no-key mode.")
                break

            page_jobs = self._parse_html(html)
            if not page_jobs:
                if page == 1:
                    log.warning("SimplyHired page returned no parseable jobs in no-key mode.")
                break
    ...
    def _parse_html(self, html: str) -> list[JobPost]:
        ...
        for script in soup.find_all("script", attrs={"type": "application/ld+json"}):
            ...
            for item in items:
                ...
                jobs.append(
                    JobPost(
                        id=f"simplyhired-{abs(hash(job_url))}",
                        title=title,
                        company_name=None,
                        job_url=job_url,

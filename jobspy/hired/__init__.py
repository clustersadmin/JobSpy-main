...
class Hired(Scraper):
    def __init__(self, proxies: list[str] | str | None = None, ca_cert: str | None = None, user_agent: str | None = None):
        super().__init__(Site.HIRED, proxies=proxies, ca_cert=ca_cert, user_agent=user_agent)
        self.base_url = "https://hired.com"
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        candidates = [
            f"{self.base_url}/jobs?search={q}&location={l}",
            f"{self.base_url}/job-search?query={q}&location={l}",
        ]
        ...
        for url in candidates:
            try:
                ...
                log.warning(f"Hired request failed: {exc}")
                continue

            if not response.ok:
                log.warning(f"Hired response status code {response.status_code}")
                continue

            html = response.text
            if "enable js" in html.lower() or "captcha" in html.lower() or "access denied" in html.lower():
                log.warning("Hired returned anti-bot/challenge page; skipping in legal no-key mode.")
                continue
    ...
    def _parse_html(self, html: str) -> list[JobPost]:
        ...
        for anchor in soup.find_all("a", href=True):
            ...
            if not any(token in href.lower() for token in ["/job", "/jobs", "/job-openings/"]):
                continue

...
log = create_logger("ComputerJobs")
...
class ComputerJobs(Scraper):
    def __init__(self, proxies: list[str] | str | None = None, ca_cert: str | None = None, user_agent: str | None = None):
        super().__init__(Site.COMPUTERJOBS, proxies=proxies, ca_cert=ca_cert, user_agent=user_agent)
        self.base_url = "https://www.computerjobs.com"
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        url = f"{self.base_url}/us/search?q={q}&l={l}"
        jobs: list[JobPost] = []
        ...
        try:
            ...
            if response.ok:
                ...
                if "captcha" not in html.lower() and "access denied" not in html.lower():
                    ...
                    log.warning("ComputerJobs returned anti-bot/challenge page; skipping in legal no-key mode.")
            else:
                log.warning(f"ComputerJobs response status code {response.status_code}")
        except Exception as exc:
            log.warning(f"ComputerJobs request failed: {exc}")
    ...
    def _parse_html(self, html: str) -> list[JobPost]:
        ...
        for script in soup.find_all("script", attrs={"type": "application/ld+json"}):
            ...
            for item in items:
                ...
                jobs.append(
                    JobPost(
                        id=f"computerjobs-{abs(hash(job_url))}",
                        title=title,
                        company_name=None,
                        job_url=job_url,
        ...
        for anchor in soup.find_all("a", href=True):
            ...
            if not any(token in href.lower() for token in ["/job", "/jobs", "/search/"]):
                continue
